...
class RaceStore:
        ...
        with self.lock, self.connection:
            cursor = self.connection.execute(
                ...
                (
                    ...
                    transport, received_at, json.dumps(clean, separators=(",", ":")),
                ),
            )
            ...
            active = self._active_row()
            if not active or clean["type"] != "crossing":
    ...
    def _validate_event(event: dict[str, Any]) -> dict[str, Any]:
        ...
        if clean["type"] not in {"crossing", "heartbeat"}:
...
class SerialBridge(threading.Thread):
    ...
    def _handle_line(self, raw_line: bytes) -> None:
        if not raw_line.startswith(b"{"):
            print(f"ESP32: {raw_line.decode('utf-8', errors='replace')}")
            return
        try:
...
def main() -> None:
    ...
    store = RaceStore(database_path)
    runtime = Runtime(store=store, bus=EventBus())
